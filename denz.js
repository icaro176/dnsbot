...
╰───「 *_ᴅɴsʙᴏᴛ_* 」`
            ...
            break  
				...
				case 'owner':
                 ...
                 denz.sendMessage(from, '*_ɪᴛᴜ ᴘᴀᴄᴀʀᴋᴜ ᴇʜ ᴍᴀᴋꜱᴜᴅɴʏᴀ ᴏᴡɴᴇʀ ᴋᴜ >-<_*',text, { quoted: mek} )
                 break
                case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							denz.sendMessage(_.jid, bcgc, image, {caption: `[ *BROADCAST GROUP* ]\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							denz.sendMessage(_.jid, bc, image, {caption: `[ *BROADCAST* ]\n\n${body.slice(4)}`})
						}
						denz.sendMessage(_.jid, bc, image, {caption: `[ *BROADCAST* ]\n\n${body.slice(4)}`})
     ...
     reply(mess.wait)
		...
		case 'jadwalsholat':
            ...
            case 'jadwaltv':
                ...
                jtv = `${anu.result}`
                denz.sendMessage(from, jtv, text, {quoted: mek})
                await limitAdd(sender)
               	break  
           ...
           case 'jadwaltvnow':
                ...
                denz.sendMessage(from, tvnow, text, {quoted: mek})
                await limitAdd(sender) 
                break 
...
case 'asupan':
				...
				reply(mess.wait)
                 ...
                 denz.sendMessage(from, asupan, video, {quoted: mek, caption: '\`\`\`ASUPAN GAN:V\`\`\`'})
				await limitAdd(sender) 
				break  
				case 'wiki':
				if (isBanned) return reply(mess.only.benned)    
		...
		case 'spamcall':
           ...
           if (isBanned) return reply(mess.only.benned)
			...
			denz.sendMessage(from, `${anu.logs}`, text, {quoted: mek})
			await limitAdd(sender) 
			break  
		case 'spamgmail':
			if (isBanned) return reply(mess.only.benned)
